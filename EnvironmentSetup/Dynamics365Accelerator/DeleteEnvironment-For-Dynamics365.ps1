...
Write-Output ("About to delete these environments...");    
$instanceno = 0;
$orgsTobeReset =""
for ($i = [int]$SuffixStart; $i -le $SuffixEnd; $i++) 
{
    $instanceno = $instanceno +1;        
    Write-Output ("$OrganizationFriendlyName$i");        
    $orgsTobeReset = "$OrganizationFriendlyName$i" + "|" + $orgsTobeReset
}

$arrOrg = $orgsTobeReset.split("|")

$ConfirmToProceed = Read-Host -Prompt "Really want to delete them... (yes or no)?"    
